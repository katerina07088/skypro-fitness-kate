export default function CoursePage() {
  return (
    <>
      <div>
        <div className="flex flex-col gap-14">
          <div className="w-full h-80 rounded-3xl bg-yoga bg-no-repeat bg-cover">
            <h3 className="text-6xl text-white font-medium leading-tight text-left p-10">
              Йога
            </h3>
          </div>
          <div>
            <h3 className="text-4xl text-black font-semibold text-left pb-10">
              Подойдет для вас, если:
            </h3>
            <div className="flex flex-row gap-x-4">
              <div className="flex gap-x-6 bg-gradient-to-r from-slate-950 to-black w-[334px] h-36 rounded-3xl p-5">
                <div className="text-lime-400 text-7xl font-medium leading-normal">
                  1
                </div>
                <p className="self-center text-left text-2xl/6 ">
                  Давно хотели попробовать йогу, но не решались начать
                </p>
              </div>
              <div className="flex gap-x-6 bg-gradient-to-r from-slate-950 to-black w-[412px] h-36 rounded-3xl p-5">
                <div className="text-lime-400 text-7xl font-medium leading-normal">
                  2
                </div>
                <p className="self-center text-left text-2xl/6">
                  Хотите укрепить позвоночник, избавиться от болей в спине и
                  суставах
                </p>
              </div>
              <div className="flex gap-x-6 bg-gradient-to-r from-slate-950 to-black w-[312px] h-36 rounded-3xl p-5">
                <div className="text-lime-400 text-7xl font-medium leading-normal">
                  3
                </div>
                <p className="self-center text-left text-2xl/6">
                  Ищете активность, полезную для тела и души
                </p>
              </div>
            </div>
          </div>

          <div>
            <h3 className="text-black text-4xl text-left font-semibold pb-10">
              Направления
            </h3>
            <div className=" bg-lime-400 text-black rounded-3xl">
              <ul role="list" className="list-inside p-7 max-w-6xl columns-3">
                <li className="flex items-center gap-2 mb-6">
                  <svg
                    width="19.5"
                    height="19.5"
                    viewBox="0 0 26 26"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M12.4637 4.36751C12.6202 3.70443 12.6984 3.3729 12.8242 3.29823C12.9326 3.23392 13.0674 3.23392 13.1758 3.29823C13.3016 3.3729 13.3798 3.70443 13.5363 4.36751L14.2997 7.60302C14.5837 8.80636 14.7257 9.40803 15.0343 9.89596C15.3071 10.3274 15.6726 10.6929 16.104 10.9657C16.592 11.2743 17.1936 11.4163 18.397 11.7003L21.6325 12.4637C22.2956 12.6202 22.6271 12.6984 22.7018 12.8242C22.7661 12.9326 22.7661 13.0674 22.7018 13.1758C22.6271 13.3016 22.2956 13.3798 21.6325 13.5363L18.397 14.2997C17.1936 14.5837 16.592 14.7257 16.104 15.0343C15.6726 15.3071 15.3071 15.6726 15.0343 16.104C14.7257 16.592 14.5837 17.1936 14.2997 18.397L13.5363 21.6325C13.3798 22.2956 13.3016 22.6271 13.1758 22.7018C13.0674 22.7661 12.9326 22.7661 12.8242 22.7018C12.6984 22.6271 12.6202 22.2956 12.4637 21.6325L11.7003 18.397C11.4163 17.1936 11.2743 16.592 10.9657 16.104C10.6929 15.6726 10.3274 15.3071 9.89596 15.0343C9.40803 14.7257 8.80636 14.5837 7.60301 14.2997L4.36751 13.5363C3.70443 13.3798 3.3729 13.3016 3.29823 13.1758C3.23392 13.0674 3.23392 12.9326 3.29823 12.8242C3.3729 12.6984 3.70443 12.6202 4.36751 12.4637L7.60302 11.7003C8.80636 11.4163 9.40803 11.2743 9.89596 10.9657C10.3274 10.6929 10.6929 10.3274 10.9657 9.89596C11.2743 9.40803 11.4163 8.80636 11.7003 7.60301L12.4637 4.36751Z"
                      fill="black"
                    />
                  </svg>
                  Йога для новичков
                </li>
                <li className="flex items-center gap-2">
                  <svg
                    width="19.5"
                    height="19.5"
                    viewBox="0 0 26 26"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M12.4637 4.36751C12.6202 3.70443 12.6984 3.3729 12.8242 3.29823C12.9326 3.23392 13.0674 3.23392 13.1758 3.29823C13.3016 3.3729 13.3798 3.70443 13.5363 4.36751L14.2997 7.60302C14.5837 8.80636 14.7257 9.40803 15.0343 9.89596C15.3071 10.3274 15.6726 10.6929 16.104 10.9657C16.592 11.2743 17.1936 11.4163 18.397 11.7003L21.6325 12.4637C22.2956 12.6202 22.6271 12.6984 22.7018 12.8242C22.7661 12.9326 22.7661 13.0674 22.7018 13.1758C22.6271 13.3016 22.2956 13.3798 21.6325 13.5363L18.397 14.2997C17.1936 14.5837 16.592 14.7257 16.104 15.0343C15.6726 15.3071 15.3071 15.6726 15.0343 16.104C14.7257 16.592 14.5837 17.1936 14.2997 18.397L13.5363 21.6325C13.3798 22.2956 13.3016 22.6271 13.1758 22.7018C13.0674 22.7661 12.9326 22.7661 12.8242 22.7018C12.6984 22.6271 12.6202 22.2956 12.4637 21.6325L11.7003 18.397C11.4163 17.1936 11.2743 16.592 10.9657 16.104C10.6929 15.6726 10.3274 15.3071 9.89596 15.0343C9.40803 14.7257 8.80636 14.5837 7.60301 14.2997L4.36751 13.5363C3.70443 13.3798 3.3729 13.3016 3.29823 13.1758C3.23392 13.0674 3.23392 12.9326 3.29823 12.8242C3.3729 12.6984 3.70443 12.6202 4.36751 12.4637L7.60302 11.7003C8.80636 11.4163 9.40803 11.2743 9.89596 10.9657C10.3274 10.6929 10.6929 10.3274 10.9657 9.89596C11.2743 9.40803 11.4163 8.80636 11.7003 7.60301L12.4637 4.36751Z"
                      fill="black"
                    />
                  </svg>
                  Классическая йога
                </li>
                <li className="flex items-center gap-2 mb-6">
                  <svg
                    width="19.5"
                    height="19.5"
                    viewBox="0 0 26 26"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M12.4637 4.36751C12.6202 3.70443 12.6984 3.3729 12.8242 3.29823C12.9326 3.23392 13.0674 3.23392 13.1758 3.29823C13.3016 3.3729 13.3798 3.70443 13.5363 4.36751L14.2997 7.60302C14.5837 8.80636 14.7257 9.40803 15.0343 9.89596C15.3071 10.3274 15.6726 10.6929 16.104 10.9657C16.592 11.2743 17.1936 11.4163 18.397 11.7003L21.6325 12.4637C22.2956 12.6202 22.6271 12.6984 22.7018 12.8242C22.7661 12.9326 22.7661 13.0674 22.7018 13.1758C22.6271 13.3016 22.2956 13.3798 21.6325 13.5363L18.397 14.2997C17.1936 14.5837 16.592 14.7257 16.104 15.0343C15.6726 15.3071 15.3071 15.6726 15.0343 16.104C14.7257 16.592 14.5837 17.1936 14.2997 18.397L13.5363 21.6325C13.3798 22.2956 13.3016 22.6271 13.1758 22.7018C13.0674 22.7661 12.9326 22.7661 12.8242 22.7018C12.6984 22.6271 12.6202 22.2956 12.4637 21.6325L11.7003 18.397C11.4163 17.1936 11.2743 16.592 10.9657 16.104C10.6929 15.6726 10.3274 15.3071 9.89596 15.0343C9.40803 14.7257 8.80636 14.5837 7.60301 14.2997L4.36751 13.5363C3.70443 13.3798 3.3729 13.3016 3.29823 13.1758C3.23392 13.0674 3.23392 12.9326 3.29823 12.8242C3.3729 12.6984 3.70443 12.6202 4.36751 12.4637L7.60302 11.7003C8.80636 11.4163 9.40803 11.2743 9.89596 10.9657C10.3274 10.6929 10.6929 10.3274 10.9657 9.89596C11.2743 9.40803 11.4163 8.80636 11.7003 7.60301L12.4637 4.36751Z"
                      fill="black"
                    />
                  </svg>
                  Кундалини-йога
                </li>
                <li className="flex items-center gap-2">
                  <svg
                    width="19.5"
                    height="19.5"
                    viewBox="0 0 26 26"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M12.4637 4.36751C12.6202 3.70443 12.6984 3.3729 12.8242 3.29823C12.9326 3.23392 13.0674 3.23392 13.1758 3.29823C13.3016 3.3729 13.3798 3.70443 13.5363 4.36751L14.2997 7.60302C14.5837 8.80636 14.7257 9.40803 15.0343 9.89596C15.3071 10.3274 15.6726 10.6929 16.104 10.9657C16.592 11.2743 17.1936 11.4163 18.397 11.7003L21.6325 12.4637C22.2956 12.6202 22.6271 12.6984 22.7018 12.8242C22.7661 12.9326 22.7661 13.0674 22.7018 13.1758C22.6271 13.3016 22.2956 13.3798 21.6325 13.5363L18.397 14.2997C17.1936 14.5837 16.592 14.7257 16.104 15.0343C15.6726 15.3071 15.3071 15.6726 15.0343 16.104C14.7257 16.592 14.5837 17.1936 14.2997 18.397L13.5363 21.6325C13.3798 22.2956 13.3016 22.6271 13.1758 22.7018C13.0674 22.7661 12.9326 22.7661 12.8242 22.7018C12.6984 22.6271 12.6202 22.2956 12.4637 21.6325L11.7003 18.397C11.4163 17.1936 11.2743 16.592 10.9657 16.104C10.6929 15.6726 10.3274 15.3071 9.89596 15.0343C9.40803 14.7257 8.80636 14.5837 7.60301 14.2997L4.36751 13.5363C3.70443 13.3798 3.3729 13.3016 3.29823 13.1758C3.23392 13.0674 3.23392 12.9326 3.29823 12.8242C3.3729 12.6984 3.70443 12.6202 4.36751 12.4637L7.60302 11.7003C8.80636 11.4163 9.40803 11.2743 9.89596 10.9657C10.3274 10.6929 10.6929 10.3274 10.9657 9.89596C11.2743 9.40803 11.4163 8.80636 11.7003 7.60301L12.4637 4.36751Z"
                      fill="black"
                    />
                  </svg>
                  Йогатерапия
                </li>
                <li className="flex items-center gap-2 mb-6">
                  <svg
                    width="19.5"
                    height="19.5"
                    viewBox="0 0 26 26"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M12.4637 4.36751C12.6202 3.70443 12.6984 3.3729 12.8242 3.29823C12.9326 3.23392 13.0674 3.23392 13.1758 3.29823C13.3016 3.3729 13.3798 3.70443 13.5363 4.36751L14.2997 7.60302C14.5837 8.80636 14.7257 9.40803 15.0343 9.89596C15.3071 10.3274 15.6726 10.6929 16.104 10.9657C16.592 11.2743 17.1936 11.4163 18.397 11.7003L21.6325 12.4637C22.2956 12.6202 22.6271 12.6984 22.7018 12.8242C22.7661 12.9326 22.7661 13.0674 22.7018 13.1758C22.6271 13.3016 22.2956 13.3798 21.6325 13.5363L18.397 14.2997C17.1936 14.5837 16.592 14.7257 16.104 15.0343C15.6726 15.3071 15.3071 15.6726 15.0343 16.104C14.7257 16.592 14.5837 17.1936 14.2997 18.397L13.5363 21.6325C13.3798 22.2956 13.3016 22.6271 13.1758 22.7018C13.0674 22.7661 12.9326 22.7661 12.8242 22.7018C12.6984 22.6271 12.6202 22.2956 12.4637 21.6325L11.7003 18.397C11.4163 17.1936 11.2743 16.592 10.9657 16.104C10.6929 15.6726 10.3274 15.3071 9.89596 15.0343C9.40803 14.7257 8.80636 14.5837 7.60301 14.2997L4.36751 13.5363C3.70443 13.3798 3.3729 13.3016 3.29823 13.1758C3.23392 13.0674 3.23392 12.9326 3.29823 12.8242C3.3729 12.6984 3.70443 12.6202 4.36751 12.4637L7.60302 11.7003C8.80636 11.4163 9.40803 11.2743 9.89596 10.9657C10.3274 10.6929 10.6929 10.3274 10.9657 9.89596C11.2743 9.40803 11.4163 8.80636 11.7003 7.60301L12.4637 4.36751Z"
                      fill="black"
                    />
                  </svg>
                  Хатха-йога
                </li>
                <li className="flex items-center gap-2">
                  <svg
                    width="19.5"
                    height="19.5"
                    viewBox="0 0 26 26"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M12.4637 4.36751C12.6202 3.70443 12.6984 3.3729 12.8242 3.29823C12.9326 3.23392 13.0674 3.23392 13.1758 3.29823C13.3016 3.3729 13.3798 3.70443 13.5363 4.36751L14.2997 7.60302C14.5837 8.80636 14.7257 9.40803 15.0343 9.89596C15.3071 10.3274 15.6726 10.6929 16.104 10.9657C16.592 11.2743 17.1936 11.4163 18.397 11.7003L21.6325 12.4637C22.2956 12.6202 22.6271 12.6984 22.7018 12.8242C22.7661 12.9326 22.7661 13.0674 22.7018 13.1758C22.6271 13.3016 22.2956 13.3798 21.6325 13.5363L18.397 14.2997C17.1936 14.5837 16.592 14.7257 16.104 15.0343C15.6726 15.3071 15.3071 15.6726 15.0343 16.104C14.7257 16.592 14.5837 17.1936 14.2997 18.397L13.5363 21.6325C13.3798 22.2956 13.3016 22.6271 13.1758 22.7018C13.0674 22.7661 12.9326 22.7661 12.8242 22.7018C12.6984 22.6271 12.6202 22.2956 12.4637 21.6325L11.7003 18.397C11.4163 17.1936 11.2743 16.592 10.9657 16.104C10.6929 15.6726 10.3274 15.3071 9.89596 15.0343C9.40803 14.7257 8.80636 14.5837 7.60301 14.2997L4.36751 13.5363C3.70443 13.3798 3.3729 13.3016 3.29823 13.1758C3.23392 13.0674 3.23392 12.9326 3.29823 12.8242C3.3729 12.6984 3.70443 12.6202 4.36751 12.4637L7.60302 11.7003C8.80636 11.4163 9.40803 11.2743 9.89596 10.9657C10.3274 10.6929 10.6929 10.3274 10.9657 9.89596C11.2743 9.40803 11.4163 8.80636 11.7003 7.60301L12.4637 4.36751Z"
                      fill="black"
                    />
                  </svg>
                  Аштанга-йога
                </li>
              </ul>
            </div>
          </div>
        </div>
        <div className="flex flex-row w-full mt-24 rounded-xl shadow-2xl">
          <div className="flex flex-col gap-7 w-[437px] m-10 h-96">
            <h3 className="text-black text-6xl font-medium text-left">
              Начните путь к новому телу
            </h3>
            <ul className="text-stone-700 text-2xl/8 font-normal list-inside list-disc text-left ">
              <li> проработка всех групп мышц</li>
              <li> тренировка суставов</li>
              <li> улучшение циркуляции крови</li>
              <li> упражнения заряжают бодростью</li>
              <li> помогают противостоять стрессам</li>
            </ul>
            <button className="rounded-3xl bg-lime-400 text-lg/[20px] text-black font-normal">
              Войдите, чтобы добавить курс
            </button>
          </div>
          <div className="relative">
            <img
              className="relative top-20 right-6 rotate-3"
              src="/public/vector.png"
              alt="greenline"
            />
            <img
              className="absolute h-[542px] w-[487px] mr-7 end-px bottom-0 "
              src="/public/addcoursepic.png"
              alt="sportsman"
            />
          </div>
        </div>
      </div>
    </>
  );
}
